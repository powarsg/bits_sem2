...
def main():
    ...
    def write_config(completed_epochs, total_time_sec, training_complete):
        config = vars(args).copy()
        config["vocab_size_actual"] = len(vocab)
        config["n_parameters"] = n_params
        config["device"] = str(device)
        config["completed_epochs"] = completed_epochs
        config["training_complete"] = training_complete
        config["total_train_time_sec"] = total_time_sec
        with open(out_dir / "train_config.json", "w") as f:
            json.dump(config, f, indent=2)

    # Keep checkpoint metadata truthful if training is interrupted. infer.py
    # only needs model-shape fields, so partial checkpoints remain usable.
    write_config(completed_epochs=0, total_time_sec=None, training_complete=False)
